...
fn test_mcp_tools_list_count() {
    ...
    assert_eq!(write_enabled.len(), 25);
    assert!(read_only.contains(&"list_admission_audit".to_string()));
    assert!(read_only.contains(&"list_runs".to_string()));
    assert!(read_only.contains(&"inspect_run".to_string()));
    assert!(read_only.contains(&"get_run_evidence".to_string()));
    assert!(read_only.contains(&"inspect_consent".to_string()));
    assert!(read_only.contains(&"estimate_review_surface".to_string()));
