...
impl<'a> PrManager<'a> {
    ...
    pub async fn create_pr(
        ...
        Err(ContribError::PrCreation(
            "admission evidence is required; use create_pr_with_evidence".to_string(),
        ))
    }

    /// Create a draft PR carrying an auditable consent and scope receipt.
    pub async fn create_pr_with_evidence(
        &mut self,
        contribution: &Contribution,
        target_repo: &Repository,
        evidence: &EvidenceCapsule,
    ) -> Result<PrResult> {
        self.create_pr_internal(contribution, target_repo, evidence)
            .await
    }
    ...
    async fn create_pr_internal(
        ...
        info!(pr = pr_number, url = %pr_url, "Draft PR created");
        Ok(result)
    }
    ...
    fn generate_pr_body(&self, contribution: &Contribution, evidence: &EvidenceCapsule) -> String {
        ...
        let evidence_markdown = evidence.to_markdown();
    ...
    pub async fn create_issue_for_finding(
        ...
        contribution: &Contribution,
        target_repo: &Repository,
        evidence: Option<&EvidenceCapsule>,
    ) -> Option<i64> {
        let Some(evidence) = evidence.filter(|value| value.repository == target_repo.full_name)
        else {
            warn!(
                repo = %target_repo.full_name,
                "Unsolicited issue creation is disabled; admission evidence is required"
            );
            return None;
        };
        ...
        warn!(
            ...
            "Creating issue under an admitted contribution permit"
        );
        let finding = &contribution.finding;
    ...
    pub async fn check_compliance_and_fix(
        ...
        if all_comments.contains("no issue referenced") || all_comments.contains("needs:issue") {
            if let Some(issue_number) = self
                .create_issue_for_finding(contribution, &pr_result.repo, None)
                .await
            {
    ...
    /// Surface CLA requests for direct human action without signing or commenting.
    pub async fn handle_cla_signing(
        ...
        for comment in cla_comments {
            ...
            // CLAAssistant still requires the contributor to attest personally.
            if login.contains("claassistant") || body_lower.contains("i have read the cla") {
                warn!(
                    pr = pr_result.pr_number,
                    repo = %repo.full_name,
                    "CLA requires direct human signature; no automated response was sent"
                );
                return;
            }
...
mod tests {
    ...
    async fn legacy_pr_creation_fails_closed_without_evidence() {
        ...
        let error = manager
            .create_pr(&test_contribution(), &test_repository())
            .await
